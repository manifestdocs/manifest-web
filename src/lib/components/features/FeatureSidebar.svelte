<script lang="ts">
  ...
  type FeatureHistory = components['schemas']['FeatureHistory'];
  type ProjectHistoryEntry = components['schemas']['ProjectHistoryEntry'];
  type HistoryEntry = FeatureHistory | ProjectHistoryEntry;
  ...
  let { feature, featureTree, projectId, projectSlug, gitRemote, acFormat }: Props =
    $props();
  ...
  const featureNode = $derived(
    feature ? findInTree(featureTree, feature.id) : null,
  );
  const isGroup = $derived(
    featureNode ? featureNode.children.length > 0 : false,
  );
              ...
              <div class="group-entries">
                {#each group.entries as entry (entry.id)}
                  <div class="history-item">
                    {#if (isGroup || isRoot) && 'feature_title' in entry}
                      ...
                      {#if entry.commits && entry.commits.length > 0}
                        <span class="commits">
                          {#each entry.commits as commit, j}
                            ...
                            {:else}
                              <code class="commit-sha" title={commit.message}
