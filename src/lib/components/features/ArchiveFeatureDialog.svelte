		...
		<Dialog.Content class="dialog-content">
			...
			<Dialog.Description class="dialog-description">
				{#if isGroup && childCount > 0}
					This feature set contains {childCount} child feature{childCount ===
					1
						? ""
						: "s"}. Choose how to handle them.
				{:else}
					Archive "{featureTitle}"? You can restore it later from the
					Archived state.
				{/if}
			</Dialog.Description>
					...
					<div class="radio-group">
						<label class="radio-option">
							...
							<span class="radio-label"
								>Archive feature set and all {childCount} child{childCount ===
								1
									? ""
									: "ren"}</span
							>
						...
						<label class="radio-option">
							...
							/>
							<span class="radio-label"
								>Move {childCount} child{childCount === 1
									? ""
									: "ren"} to parent and archive feature set</span
							>
						</label>
					</div>
				{/if}
