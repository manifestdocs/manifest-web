<script lang="ts">
	...
	const iconPaths: Record<string, string> = {
		...
		'test-tube': 'M9 2v10l-4.5 7a2 2 0 001.7 3h11.6a2 2 0 001.7-3L15 12V2M8 2h8M7 16h10',
...
{#if matches.length > 0}
	...
	<div class="command-menu" onmousedown={(e) => e.preventDefault()}>
		{#each matches as match, i (match.command.name)}
			{@const segments = getMatchHighlights(match.command.name, match.highlights)}
			<button
				class="command-item"
			...
			>
				...
				<span class="command-name">
					<span class="command-slash">/</span>{#each segments as seg}{#if seg.highlighted}<strong>{seg.text}</strong>{:else}{seg.text}{/if}{/each}
				</span>
