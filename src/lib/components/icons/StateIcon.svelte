<script lang="ts">
  ...
  let { state, size = 16, hasChanges = false, class: className = '' }: Props = $props();
</script>
...
{:else if state === 'implemented' && hasChanges}
...
{:else if state === 'implemented'}
  ...
  >
    <circle cx="8" cy="8" r="7" fill="var(--state-implemented)" />
    <path
