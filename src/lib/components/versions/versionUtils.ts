...
export function getNextVersionName(versions: Version[]): string {
  ...
  const highest = parsed.reduce((max, v) => {
    ...
    if (v.major === max.major && v.minor === max.minor && v.patch > max.patch)
      return v;
    return max;
  });
...
export function groupVersions(versions: Version[]): VersionGroup[] {
  ...
  if (unreleased.length > 0) {
    groups.push({ label: 'Next', versions: [unreleased[0]] });
  }
  if (unreleased.length > 1) {
    groups.push({ label: 'Planned', versions: unreleased.slice(1) });
  }

  return groups;
}

export function getVersionFlatIndex(
...
export function getTotalVersionColumns(
  groupedVersions: VersionGroup[],
): number {
