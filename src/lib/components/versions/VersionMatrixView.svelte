<script lang="ts">
	...
	let activeFilters = $state<Set<import('$lib/stores/featureFilter.svelte.js').FilterableState>>(new Set());
	...
	async function handleDotDrop(
		...
		const resolvedVersionId =
			newVersionId === "backlog" ? null : newVersionId;
	...
	function handleToggleFilter(state: import('$lib/stores/featureFilter.svelte.js').FilterableState) {
...
>
	...
	{#if isNowFeatureComplete && onCompleteVersion && nowVersion}
		...
		>
			...
			{/snippet}
			<strong>{nowVersion.name}</strong> is feature complete. Close and
			advance planning?
			<button
				type="button"
				class="banner-btn"
