...
<p>
  Versions let you group features into releases and track progress toward
  milestones. Manifest uses semantic versioning (0.1, 0.2, 1.0) and organizes
  work with a Now / Next / Later model.
</p>
...
<Admonition type="skill" title="Claude Code slash command /manifest:versions">
  {#snippet children()}
    <p>
      Use <code>/manifest:versions</code> to display the version roadmap with now/next/later
...
<p>
  Use semantic versioning: <code>0.1</code>, <code>0.2</code>, <code>1.0</code>.
  You can also use names like <code>MVP</code> or <code>beta</code>.
</p>
...
<Admonition
  type="skill"
  title="Claude Code slash command /manifest:assign [feature] [version]"
>
  {#snippet children()}
    <p>
      Use <code>/manifest:assign OAuth 0.2</code> to schedule a feature for a specific
      release.
    </p>
  {/snippet}
</Admonition>
...
<ol>
  <li>
    <strong>Now version first:</strong> Features in the current release get priority
  </li>
  <li>
    <strong>Priority within version:</strong> Lower priority number = higher importance
  </li>
  <li>
    <strong>Backlog last:</strong> Unassigned features after versioned work
  </li>
</ol>

<p>
  This means when you ask "What should we work on next?", the agent pulls from
...
<p>
  When all features in a version are implemented, the agent automatically closes
  it:
</p>
...
<p>
  This keeps planning simple: finish what's in Now, then Now becomes the next
...
<Admonition
  type="skill"
  title="Claude Code slash command /manifest:release [version]"
>
  {#snippet children()}
    <p>
      Use <code>/manifest:release 0.2</code> to mark a version as shipped, or
      <code>/manifest:release</code> to release the current "now" version.
    </p>
  {/snippet}
</Admonition>
